...
public class PopupMenuPanel extends JPanel{
    ...
    private GameButton max;
    private GameButton min;
...
//    public final static String TOP_10_SCORES_BUTTON = "TOP_10_SCORES_BUTTON";
    ...
    private void initComponents(MainFrame mainFrame) {
    	...
    	this.logoLabel.setIcon(new ImageIcon(Images.MY_Ant_IMG));

    	this.max = new GameButton("Max Time");
    	this.max.addActionListener(mainFrame);
    	this.max.setActionCommand(MAX);
    	this.max.setOpaque(false);
    	
    	this.min = new GameButton("Min Time");
    	this.min.addActionListener(mainFrame);
    	this.min.setActionCommand(MIN);
    	this.min.setOpaque(false);
    	
    	this.startGameButton = new GameButton("Optional");
